export class Datetime {
    ...
    getCurrentDateTime(dateSeparateSymbol: string = '-') {
        ...
        let milseconds : string = this.formatWithTwoDigits(String(dateTime.getMilliseconds()));

        return `${dateTime.getFullYear()}${dateSeparateSymbol}${month}${dateSeparateSymbol}${dateDay} ${hour}:${minutes}:${seconds}:${milseconds}`;
    }
